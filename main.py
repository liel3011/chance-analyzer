    ...
    with tab_sleep:
        ...
        for col_idx, col_name in enumerate(required_cols):
            if sleep_window_start + 1 >= len(grid_data):
                col_data_past = np.array([])
            else:
                col_data_past = grid_data[sleep_window_start + 1:, col_idx]
                
            c_unique_past = np.unique(grid_data[:, col_idx].astype(str))
            for c in c_unique_past:
                if str(c).lower() == 'nan' or not str(c).strip(): continue
                locs_past = np.where(col_data_past == str(c))[0]
                gap_past = locs_past[0] if len(locs_past) > 0 else len(col_data_past)
                if gap_past >= sleep_threshold: 
                    past_sleepers.add((col_idx, str(c)))

        for col_idx, col_name in enumerate(required_cols):
            if sleep_window_start >= len(grid_data):
                col_data_curr = np.array([])
            else:
                col_data_curr = grid_data[sleep_window_start:, col_idx]
                
            c_unique_curr = np.unique(grid_data[:, col_idx].astype(str))
            lst = []
            for c in c_unique_curr:
                if str(c).lower() == 'nan' or not str(c).strip(): continue
                locs_curr = np.where(col_data_curr == str(c))[0]
                gap_curr = locs_curr[0] if len(locs_curr) > 0 else len(col_data_curr)
                if gap_curr >= sleep_threshold: 
                    lst.append((c, gap_curr))
            lst.sort(key=lambda x: x[1], reverse=True)
            sleep_data_lists[col_name] = [f"{item[0]} : {item[1]}" for item in lst]

        st.markdown(f"<h4 style='margin-top: 15px; font-weight: 800; color: #F3F4F6;'>Sleeping Cards (CURRENT state from Row {sleep_window_start})</h4>", unsafe_allow_html=True)
        st.markdown("<div style='color: #9CA3AF; font-size: 13px; margin-bottom: 10px;'>If a card was drawn in the target row, it is no longer sleeping and won't appear here.</div>", unsafe_allow_html=True)
        ...
        st.markdown("<div style='color: #9CA3AF; font-size: 13px; margin-bottom: 10px;'>Green highlighted cells indicate cards that were sleeping BEFORE the target draw, and exactly when they woke up.</div>", unsafe_allow_html=True)
        
        cell_classes_sleep = {}
        end_draw = max(0, sleep_window_start - 2)
        ...
        for r in range(sleep_window_start, end_draw - 1, -1):
            for c in range(4):
                ...
                if (c, val) in past_sleepers:
                    classes += " awakened-card"
                    past_sleepers.remove((c, val)) 
                    
                if classes:
                    cell_classes_sleep[(r, c)] = classes.strip()
                    
        st.markdown(generate_board_html(grid_data, end_draw, draw_limit_sleep, cell_classes_sleep), unsafe_allow_html=True)
