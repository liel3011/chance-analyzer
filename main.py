...
def normalize_shape(shape):
    """מנרמל את הקואורדינטות כך שהצורה תתחיל מ-(0,0) לאחר סיבוב/היפוך"""
...
def generate_variations_strict(shape_idx, base_shape):
    ...
    # 8 Symmetries of a geometric shape
    ...
    transp1 = normalize_shape([(c, r) for r, c in base_shape]) # Main diagonal reflection
    transp2 = normalize_shape([(w - c, h - r) for r, c in base_shape]) # Anti-diagonal reflection
    ...
    if shape_idx in [0, 1]:
        ...
        # ALL DIRECTIONS (Includes 90/270 degree rotations and transpositions)
        variations.update([mirror_h, flip_v, rot_180, rot_90, rot_270, transp1, transp2])
        
    # מסנן רק צורות שפיזית יכולות להיכנס ללוח של 4 טורים
    valid_variations = []
    for v in variations:
