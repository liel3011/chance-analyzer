...
def generate_variations_strict(shape_idx, base_shape):
    variations = set()
    base = normalize_shape(base_shape)
    variations.add(base)
    ...
    mirror_h = normalize_shape([(r, w - c) for r, c in base_shape])
    flip_v = normalize_shape([(h - r, c) for r, c in base_shape])
    rot_180 = normalize_shape([(h - r, w - c) for r, c in base_shape])
    
    rot_90 = normalize_shape([(c, h - r) for r, c in base_shape])
    rot_270 = normalize_shape([(w - c, r) for r, c in base_shape])
    transp1 = normalize_shape([(c, r) for r, c in base_shape]) 
    transp2 = normalize_shape([(w - c, h - r) for r, c in base_shape]) 
...
<style>
    ...
    .stApp { 
        direction: ltr; 
        text-align: left; 
        background-color: #0B0F19; /* Deep midnight theme */
        color: #F3F4F6; 
        font-family: 'Inter', sans-serif;
    }
    ...
    /* Accent Colors for +/- Mode */
    .cell-plus { color: #10B981 !important; font-weight: 800 !important; text-shadow: 0 0 8px rgba(16, 185, 129, 0.4); } 
    .cell-minus { color: #F43F5E !important; font-weight: 800 !important; text-shadow: 0 0 8px rgba(244, 63, 94, 0.4); } 
    
    /* Glowing Missing Marker */
    .missing-circle { 
        background: linear-gradient(135deg, #F59E0B 0%, #D97706 100%); 
        color: #FFFFFF; 
        font-weight: 800; 
...
def create_sleeping_html_table(data_dict, required_cols):
    meta = {
        'Clubs': {'icon': '♣', 'color': '#D1D5DB'},    # Sleek Gray
        'Diamonds': {'icon': '♦', 'color': '#EF4444'}, # Vibrant Red
        'Hearts': {'icon': '♥', 'color': '#EF4444'},   
        'Spades': {'icon': '♠', 'color': '#D1D5DB'}
    }
...
def find_matches_for_pattern(shape_idx, selected_cards, grid_data, row_limit):
    ...
    variations = generate_variations_strict(shape_idx, base_shapes[shape_idx])
    rows = min(len(grid_data), row_limit)
    # Modern Neon Palette for borders
    colors = ['#3B82F6', '#8B5CF6', '#10B981', '#F59E0B', '#EF4444']
