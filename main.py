...
with tab_hist_pred:
    ...
    st.markdown("Evaluates the last 200 draws. Draws where the **Top 3 predicted cards** matched the actual outcome in **3 or 4 suits** will have their winning cards highlighted in <span style='color:#60A5FA; font-weight:bold;'>BLUE</span>.", unsafe_allow_html=True)
    ...
    if st.button("🚀 Run Predictor Backtest", key="btn_run_bt_pred"):
        with st.spinner("Crunching historical data..."):
            ...
            for r in range(min(200, len(grid_data))):
                for c in range(4): bt_classes[(r, c)] = "window-dim"
                ...
                for r in range(1, min(201, len(grid_data)-2)):
                    ...
                    for c in range(4):
                        ...
                        if misses:
                            ...
                            if str(grid_data[r-1, c]).strip().upper() in top_3: 
                                hit_cols.append(c)
                    
                    if len(hit_cols) >= 3:
                        wins += 1
                        for c in hit_cols: 
                            bt_classes[(r-1, c)] = "history-hit-3row"
