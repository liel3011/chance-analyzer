...
if df is not None:
    ...
    with tab_predictor:
        ...
        with st.expander("⚙️ Pattern Filter Checklist", expanded=False):
            st.markdown("<div style='color: #9CA3AF; font-size: 13px; margin-bottom: 10px;'>Check patterns to include them in the prediction calculation:</div>", unsafe_allow_html=True)
            ...
            for k, v in items[:mid]:
                if col1.checkbox(v, value=False, key=f"chk_pat_{k}"):
                    active_pattern_indices.append(k)
                    
            for k, v in items[mid:]:
                if col2.checkbox(v, value=False, key=f"chk_pat_{k}"):
                    active_pattern_indices.append(k)
        st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)
