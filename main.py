...
def load_data_bulletproof(uploaded_file):
    ...
    try:
        ...
        except: return None, "Error loading file"
    ...
    for col in SUITS:
        if col in df.columns: df[col] = df[col].apply(strict_card_mapper)
    return df, "ok"
...
def generate_variations_strict(shape_idx, base_shape):
    ...
    for b in bases:
        ...
        for t in valid_transforms:
            norm_t = normalize(t)
            if max(c for r, c in norm_t) < 4: variations.add(norm_t)
    ...
    for shape in variations:
        ...
        for r in range(rows - sh_h + 1):
            for c in range(4 - sh_w + 1):
                ...
                temp_selected, used_indices = selected_cards.copy(), []
...
with st.sidebar:
    ...
    global_sleep_thresh = st.number_input("💤 Sleep Threshold", min_value=3, max_value=20, value=7, step=1)
...
# ==========================================
tab_pred, tab_sleepers, tab_matches, tab_lab = st.tabs([
    "🔮 SMART 3-ROW PREDICTOR", 
    "💤 SLEEPERS RADAR",
    "📋 PATTERN MATCHES", 
    "🧪 PATTERN LAB"
])
...
# ------------------------------------------
# TAB 1: SMART PREDICTOR (LIVE & BACKTEST)
# ------------------------------------------
with tab_pred:
        ...
        with suit_tabs[i]:
            ...
            if len(triplet) == 3 and active_pats:
                ...
                for p_idx in active_pats:
                    all_missing.extend([x['miss_val'].strip().upper() for x in find_matches_for_pattern(p_idx, triplet, historical_scan, global_search_depth)])
                if all_missing:
    ...
    st.markdown("---")
    st.markdown("<h3 style='color: #F9FAFB;'>📈 Historical Backtest (Top-3 Hits)</h3>", unsafe_allow_html=True)
    
    if st.button("🚀 Run Predictor Backtest", key="btn_run_bt_pred"):
        with st.spinner("Crunching historical data..."):
            bt_rows = []
            wins = 0
            if not active_pats: st.error("Please select patterns above.")
            else:
                for r in range(1, min(201, len(grid_data)-2)):
                    ...
                    if hit_count >= 3:
                        ...
                        bt_rows.append({
                            "Row": r - 1,
                            "♠ Spades": str(grid_data[r-1, 0]).replace('nan','-'),
                            "♥ Hearts": str(grid_data[r-1, 1]).replace('nan','-'),
                            "♦ Diamonds": str(grid_data[r-1, 2]).replace('nan','-'),
                            "♣ Clubs": str(grid_data[r-1, 3]).replace('nan','-'),
                            "Hits (Top 3)": hit_count
                        })
                st.session_state['bt_pred_df'] = pd.DataFrame(bt_rows)
                st.session_state['bt_pred_wins'] = wins
                st.session_state['bt_pred_run'] = True

    if st.session_state.get('bt_pred_run', False):
        st.success(f"**Found {st.session_state['bt_pred_wins']} draws** with 3 or 4 hits in the last 200 draws.")
        st.markdown("**🔍 Interactive Time Machine:** Click on any row below to instantly view that draw in the tools above!")
        
        df_pred = st.session_state['bt_pred_df']
        if not df_pred.empty:
            event_pred = st.dataframe(df_pred, use_container_width=True, hide_index=True, selection_mode="single-row", on_select="rerun", key="df_pred_bt")
            if len(event_pred.selection['rows']) > 0:
                t_row = int(df_pred.iloc[event_pred.selection['rows'][0]]['Row']) + 1
                if st.session_state['window_start'] != t_row:
                    st.session_state['window_start'] = t_row
                    st.rerun()
...
with tab_sleepers:
    ...
    title_txt = "Right Now (Heading into Next Draw)" if sw_start == 0 else f"Just Before Row {sw_start - 1}"
    st.markdown(f"<h4 style='margin-top: 15px; font-weight: 800; color: #F3F4F6;'>Sleeping Cards ({title_txt})</h4>", unsafe_allow_html=True)
    if any(slp_data.values()): st.markdown(create_sleeping_html_table(slp_data, SUITS), unsafe_allow_html=True)
    ...
    if sw_start == 0:
        ...
        woke_target = [(c, str(grid_data[target_draw, c])) for c in range(4) if (c, str(grid_data[target_draw, c])) in active_sleepers]
        
        for r in range(sw_start - 1, -1, -1):
            for c in range(4):
                val = str(grid_data[r, c])
                classes = "trigger-row " if r == target_draw else ""
                if r == target_draw and len(woke_target) >= 3 and (c, val) in woke_target: classes += "awakened-card"
    ...
    st.markdown("---")
    st.markdown("<h3 style='color: #F9FAFB;'>📉 Historical Backtest (Chain Reactions)</h3>", unsafe_allow_html=True)
    ...
    if st.button("🚀 Analyze Past 200 Draws", key="btn_run_slp_hist"):
        with st.spinner("Analyzing chain reactions..."):
            ...
            st.session_state['bt_slp_wins'] = wins_sleep
    ...
    if st.session_state.get('sleep_hist_run', False):
        st.success(f"**Found {st.session_state['bt_slp_wins']} draws** with 3 or 4 mass awakenings.")
        st.markdown("**🔍 Interactive Time Machine:** Click on any row below to instantly view the Radar at that exact moment!")
        
        df_slp = st.session_state['sleep_hist_df']
        if not df_slp.empty:
            event_slp = st.dataframe(df_slp, use_container_width=True, hide_index=True, selection_mode="single-row", on_select="rerun", key="df_slp_bt")
            if len(event_slp.selection['rows']) > 0:
                t_row = int(df_slp.iloc[event_slp.selection['rows'][0]]['Row']) + 1
                if st.session_state['sleep_window_start'] != t_row:
                    st.session_state['sleep_window_start'] = t_row
                    st.rerun()
...
with tab_matches:
    with st.expander("⚙️ Configuration & Target Inputs", expanded=not st.session_state.get('search_done', False)):
        ...
        with col_conf:
            ...
            with nav_col1: st.button("◀", use_container_width=True, key="prev_pat_btn_matches", on_click=prev_pat)
            with nav_col2:
            ...
            with nav_col3: st.button("▶", use_container_width=True, key="next_pat_btn_matches", on_click=next_pat)
        ...
        with b_search: run_search = st.button("🔍 Search", type="primary", use_container_width=True, key="search_matches")
        with b_reset: st.button("Reset", use_container_width=True, on_click=reset_search, key="res_matches")
...
with tab_lab:
    ...
    if st.button("🔍 Search History", type="primary", key="search_lab"):
        if len(sel_cards) == 3:
